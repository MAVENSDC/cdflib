...
class CDFEpochTestCase(unittest.TestCase):
    ...
    def test_encode_cdfepoch(self):
        x = cdflib.epochs.encode([62285326000000.0,62985326000000.0])
        self.assertEqual(x[0], '1973-09-28T23:26:40.000')
        self.assertEqual(x[1], '1995-12-04T19:53:20.000')
        y = cdflib.epochs.encode(62975326000002.0, iso_8601=False)
    ...
    def test_encode_cdfepoch16(self):
        ...
        """
        x = cdflib.epochs.encode(np.complex128(63300946758.000000 + 176214648000.00000j))
        self.assertEqual(x, '2005-12-04T20:19:18.176214648000')
        y = cdflib.epochs.encode(np.complex128([33300946758.000000 + 106014648000.00000j,
    ...
    def test_encode_cdftt2000(self):
        x = cdflib.epochs.encode(186999622360321123)
        self.assertEqual(x, '2005-12-04T20:19:18.176321123')
        x1 = cdflib.epochs.encode(186999622360321123, iso_8601=False)
        self.assertEqual(x1, '2005-12-04 20:19:18.176.321.123')
        y = cdflib.epochs.encode([500000000100, 123456789101112131], iso_8601=False)
        self.assertEqual(y[0], '2000-01-01 12:07:15.816.000.100')
        self.assertEqual(y[1], '2003-11-30 09:32:04.917.112.131')

    def test_unixtime(self):
        x = cdflib.epochs.unixtime([500000000100, 123456789101112131])
        y = cdflib.epochs.unixtime(62985326000002.0)
        z = cdflib.epochs.unixtime(np.complex128(63300946758.000000 + 176214648000.00000j), to_np=True)
    ...
    def test_breakdown_cdfepoch(self):
        x = cdflib.epochs.breakdown([62285326000000.0, 62985326000000.0])
        # First in the array
    ...
    def test_breakdown_cdfepoch16(self):
        x = cdflib.epochs.breakdown(np.complex128(63300946758.000000 + 176214648000.00000j))
    ...
    def test_breakdown_cdftt2000(self):
        x = cdflib.epochs.breakdown(123456789101112131)
    ...
    def test_compute_cdfepoch(self):
        ...
        x = cdflib.epochs.breakdown(cdflib.epochs.compute(random_time))
        i = 0
        for t in x:
    ...
    def test_findepochrange_cdfepoch(self):
        ...
        x = cdflib.epochs.parse([start_time, end_time])
        time_array = np.arange(x[0], x[1], step=1000000)
        test_start = [2014, 8, 1, 8, 1, 54, 123]
        test_end = [2018, 1, 1, 1, 1, 1, 1]
        index = cdflib.epochs.findepochrange(time_array, starttime=test_start, endtime=test_end)
        # Test that the test_start is less than the first index, but more than one less
        self.assertGreaterEqual(time_array[index[0]], cdflib.epochs.compute(test_start))
        self.assertLessEqual(time_array[index[0]-1], cdflib.epochs.compute(test_start))

        self.assertLessEqual(time_array[index[-1]], cdflib.epochs.compute(test_end))
        return
    ...
    def test_findepochrange_cdftt2000(self):
        ...
        self.assertLessEqual(time_array[index[-1]], cdflib.epochs.compute(test_end))
        self.assertGreaterEqual(time_array[index[-1]+1], cdflib.epochs.compute(test_end))
        return
    ...
    def test_findepochrange_cdfepoch16(self):
        ...
        # Test that the test_start is less than the first index, but more than one less
        self.assertGreaterEqual(time_array[index[0]].real, cdflib.epochs.compute(test_start).real)
        self.assertLessEqual(time_array[index[0]-1].real, cdflib.epochs.compute(test_start).real)
        self.assertLessEqual(time_array[index[-1]].real, cdflib.epochs.compute(test_end).real)
        self.assertGreaterEqual(time_array[index[-1]+1].real, cdflib.epochs.compute(test_end).real)
        return
