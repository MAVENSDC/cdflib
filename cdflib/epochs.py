...
and TT2000 is 8-byte integer (as int in Python). In Numpy, they are np.float64, np.complex128 and np.int64,
respectively. All these epoch values can come from CDF.varget function.

Five main functions are provided 

encode (epochs, iso_8601=True, unixtime=False)
=============
...
def encode(epochs, iso_8601=True, unixtime=False):  # @NoSelf
    if isinstance(epochs, int) or isinstance(epochs, np.int64):
        return encode_tt2000(epochs, iso_8601, unixtime)
    elif isinstance(epochs, float) or isinstance(epochs, np.float64):
        return encode_epoch(epochs, iso_8601, unixtime)
    elif isinstance(epochs, complex) or isinstance(epochs, np.complex128):
        return encode_epoch16(epochs, iso_8601, unixtime)
...
def breakdown(epochs, to_np=None):  # @NoSelf
    ...
    if isinstance(epochs, int) or isinstance(epochs, np.int64):
        return breakdown_tt2000(epochs, to_np)
    elif isinstance(epochs, float) or isinstance(epochs, np.float64):
...
def unixtime(cdf_time, to_np=False):  # @NoSelf
    unix_times = encode(cdf_time, iso_8601=False, unixtime=True)
    if to_np:
...
def compute(datetimes, to_np=None):  # @NoSelf
    if (not isinstance(datetimes, list) and not isinstance(datetimes, tuple) and not
    isinstance(datetimes, np.ndarray)):
        print('datetime must be in list form')
        return
    if isinstance(datetimes[0], numbers.Number):
        items = len(datetimes)
    elif isinstance(datetimes[0], list) or isinstance(datetimes[0], tuple) or isinstance(datetimes[0], np.ndarray):
    ...
    if items == 7:
        ...
        return compute_tt2000(datetimes, to_np)
    else:
        print('Unknown input')
        return
...
def findepochrange(epochs, starttime=None, endtime=None):  # @NoSelf
    if isinstance(epochs, float) or isinstance(epochs, np.float64):
        return epochrange_epoch(epochs, starttime, endtime)
    elif isinstance(epochs, int) or isinstance(epochs, np.int64):
        return epochrange_tt2000(epochs, starttime, endtime)
    elif isinstance(epochs, complex) or isinstance(epochs, np.complex128):
        return epochrange_epoch16(epochs, starttime, endtime)
    elif isinstance(epochs, list) or isinstance(epochs, tuple) or  isinstance(epochs, np.ndarray):
...
def encode_tt2000(tt2000, iso_8601=True, unixtime=False):  # @NoSelf
    ...
    encoded = list()
    for x in new_tt2000:
        epoch, nansec = breakdown_tt2000(x, encoding=True)
...
def breakdown_tt2000(tt2000, to_np=None, encoding=None):  # @NoSelf
    ...
    for x in range(0, count):
        ...
        datx = _LeapSecondsfromJ2000(nanoSecSinceJ2000)
        if nanoSecSinceJ2000 > 0:
            secSinceJ2000 = int(nanoSecSinceJ2000 /SECinNanoSecsD)
            nansec = int(nanoSecSinceJ2000 - secSinceJ2000 * SECinNanoSecs)
            secSinceJ2000 = secSinceJ2000 - 32
            secSinceJ2000 = secSinceJ2000 + 43200
            nansec = nansec - 184000000
        else:
            nanoSecSinceJ2000 = nanoSecSinceJ2000 + T12hinNanoSecs
            nanoSecSinceJ2000 = nanoSecSinceJ2000 - dTinNanoSecs
            secSinceJ2000 = int(nanoSecSinceJ2000 / SECinNanoSecsD)
            nansec = int(nanoSecSinceJ2000 - secSinceJ2000 * SECinNanoSecs)
        if nansec < 0:
        ...
        if datx[0] > 0.0:
            ...
            tmpNanosecs = compute_tt2000(xdate1)
            if tmpNanosecs != t3:
                dat0 = _LeapSecondsfromYMD(xdate1[0],
                                                    xdate1[1], xdate1[2])
                tmpx = t2 - int(dat0 * SECinNanoSecs)
                tmpy = int(float(tmpx / SECinNanoSecsD))
                nansec = int(tmpx - tmpy * SECinNanoSecs)
            if nansec < 0:
                nansec = SECinNanoSecs + nansec
                tmpy -= 1
                epoch = tmpy + J2000Since0AD12hSec
                xdate1 = _EPOCHbreakdownTT2000(epoch)
                xdate1.append(0)
                xdate1.append(0)
                xdate1.append(nansec)
            ...
            if tmpNanosecs != t3:
                dat0 = _LeapSecondsfromYMD(xdate1[0],
                                                    xdate1[1], xdate1[2])
                tmpx = t2 - int(dat0 * SECinNanoSecs)
                tmpy = int((1.0 * tmpx) / SECinNanoSecsD)
                nansec = int(tmpx - tmpy * SECinNanoSecs)
                if nansec < 0:
                    nansec = SECinNanoSecs + nansec
                    tmpy = tmpy - 1
...
def compute_tt2000(datetimes, to_np=None):  # @NoSelf
    global currentDay, currentJDay, currentLeapSeconds
    ...
    for x in range(0, count):
        ...
        if year == 9999 and month == 12 and day == 31 and hour == 23 and minute == 59 and second == 59 and \
            ...
            if iy != currentDay:
                currentDay = iy
                currentLeapSeconds = _LeapSecondsfromYMD(year, month, day)
                currentJDay = _JulianDay(year, month, day)
            jd = currentJDay
            jd = jd - JulianDateJ2000_12h
            subDayinNanoSecs = int(hour * HOURinNanoSecs +
                                   minute * MINUTEinNanoSecs +
                                   second * SECinNanoSecs + msec * 1000000 +
                                   usec * 1000 + nsec)
            nanoSecSinceJ2000 = int(jd * DAYinNanoSecs +
                                    subDayinNanoSecs)
            t2 = int(currentLeapSeconds * SECinNanoSecs)
            if nanoSecSinceJ2000 < 0:
...
def _LeapSecondsfromYMD(year, month, day):  # @NoSelf
    j = -1
    m = 12 * year + month
    for i, _ in reversed(list(enumerate(LTS))):
        n = 12 * LTS[i][0] + LTS[i][1]
        if m >= n:
            j = i
            break
    if j == -1:
    ...
    if j < NERA1:
        ...
        da = da + ((jda - MJDbase) - LTS[j][4]) * LTS[j][5]
    return da
...
def _LoadLeapNanoSecondsTable():  # @NoSelf
    NST = list()
    for ix in range(0, NERA1):
        NST.append(FILLED_TT2000_VALUE)
    for ix in range(NERA1, NDAT):
        NST.append(compute_tt2000([int(LTS[ix][0]), int(LTS[ix][1]), int(LTS[ix][2]), 0, 0, 0, 0, 0, 0]))
    return NST
...
def epochrange_tt2000(epochs, starttime=None, endtime=None):  # @NoSelf
    ...
    if starttime is None:
        ...
        if isinstance(starttime, int) or isinstance(starttime, np.int64):
            ...
            stime = compute_tt2000(starttime)
        else:
...
def compute_epoch16(datetimes, to_np=None):  # @NoSelf
    ...
    for x in range(0, count):
        ...
        if (year == 9999 and month == 12 and day == 31 and hour == 23 and minute == 59 and second == 59 and
                ...
                epoch.append(-1.0E31)
                epoch.append(-1.0E31)
        elif ((year > 9999) or (month < 0 or month > 12) or (hour < 0 or hour > 23) or (minute < 0 or minute > 59)
            ...
            if month == 0:
                if day < 1 or day > 366:
                    epoch = _computeEpoch16(year, month, day, hour, minute, second, msec, usec, nsec, psec)
            else:
                if day < 1 or day > 31:
                    epoch = _computeEpoch16(year, month, day, hour, minute, second, msec, usec, nsec, psec)
            if month == 0:
    ...
    if not isinstance(value, list) and not isinstance(value, tuple) and not isinstance(value, str):
        ...
        if isinstance(value, list) or isinstance(value, tuple):
            ...
            for x in range(0, num):
                epochs.append(_parse_epoch(value[x]))
            if to_np is None:
                return epochs
            else:
                return np.array(epochs)
            ...
            if to_np is None:
                ...
                return np.array(_parse_epoch(value))
        ...
        if len(value) == 23:
            ...
            if value == '9999-12-31t23:59:59.999999999' or value == '9999-12-31 23:59:59.999.999.999':
                ...
                if len(date[0]) == 7:
                    ...
                    us = int(date[0][7])
                    ns = int(date[0][8])
            return compute_tt2000([yy, mm, dd, hh, mn, ss, ms, us, ns])
        else:
