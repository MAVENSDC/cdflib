...
def FileOrUrlOrS3Handler(fname,ftype):
    if ftype == 'url':
        ...
        bdata = boto3.resource('s3').Object(mybucket,mykey)
        #s3c = boto3.client('s3')
...
class S3object:
    ...
    def read(self,isize):
        myrange = 'bytes='+str(self.pos)+'-'+str(self.pos+isize-1)
...
class CDF:
    ...
    def __init__(self, path, validate=False, string_encoding='ascii'):
        ...
        self._f = FileOrUrlOrS3Handler(self.file,self.ftype) # S3-aware
        if self.ftype == 's3':
            self._f = S3object(self._f)
            
        magic_number = self._f.read(4).hex()
        #print("debug: magic number ",magic_number)
        ...
        # S3
        # right now both urls and compressed S3 make a temp copy
        # non-compressed S3 use more efficient 'read in place'
        if self.ftype == 'url':
            raw_data = self._f.read(-1)
            self._unstream_file(raw_data)
            path=self.file
            #print("debug: new temp file is: ",path)
        elif self.ftype == 's3' and self._compressed:
            raw_data = self._f.fetchS3entire()
            self._unstream_file(raw_data)
            path=self.file
    ...
    def __del__(self):
        ...
        try:
            ...
            pass # s3 objects do not like close
    ...
    def _unstream_file(self, raw_data):
        """
